...
(eval-when (:load-toplevel)
  ...
  (defparameter *globalptr-symbols*
    (with-open-file (in (string+ (directory-namestring *load-pathname*) "globalptr.sexp"))
...
(defun startup ()
  (labels ((gen ()
             `(lambda ()
                (progn ,@(loop for sym in *globalptr-symbols*
                               collect
                               #+(version>= 11 0)
                               `(def-foreign-constant ,sym :type :unsigned-nat)
                               #-(version>= 11 0)
                               `(def-foreign-variable ,sym :type :unsigned-natural))))))
    (funcall (gen))
    t))
...
(eval-when (:load-toplevel)
  (defvar-nonbindable *pycontext* (init-pycontext)))
